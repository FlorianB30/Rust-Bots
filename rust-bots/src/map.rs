...
impl Map {
    ...
    pub fn display(&self) {
        ...
        for row in &self.grid {
            for cell in row {
                let symbol = match cell {
                    Cell::Empty => Colour::Fixed(235).paint("."),  
                    Cell::Obstacle => Colour::Fixed(160).paint("#"),  
                    Cell::Energy => Colour::Green.paint("E"),  
                    Cell::Mineral => Colour::Fixed(208).paint("M"),  
                    Cell::ScienceSpot => Colour::Blue.paint("S"),  
                    Cell::Bot => Colour::White.paint("O"), 
                };
                print!("{}", symbol);
            }
            println!();
